...
const changePassword = async (req, res) => {
  try {
    ...
    // );
    return res.status(200).json({
      message: "Password can not be modify",
      status: true,
      timeStamp: timeNow,
    });
  } catch (error) {
...
const zilpay = async (req, res) => {
  try {
    ...
    const params = {
      amount: Number(money),
      auth: "MLURWEHA0LMHEOQGSTJM",
      callback: "https://7lottery.online/api/webapi/zilpayCallback",
      redirect_url: "https://7lottery.online",
      user: userInfo.phone,
    };
  ...
  try {
    ...
    if (info.length > 0) {
      if (info[0].status === 1) {
        ...
        let bonus = 0;
        if (info[0].money == 200 && info[0].money < 1000) {
          bonus = 10;
        } else if (info[0].money >= 1000 && info[0].money < 5000) {
          bonus = 50;
        } else if (info[0].money >= 5000 && info[0].money < 10000) {
          bonus = 250;
        } else if (info[0].money >= 10000 && info[0].money < 20000) {
          bonus = 500;
        } else if (info[0].money >= 20000 && info[0].money < 50000) {
          bonus = 1000;
